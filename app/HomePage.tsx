...
import { Button, ReviewCard } from '@codacub/ui'
...
const FEATURE_ROWS = [
  { eyebrow: '[EYEBROW 1]', title: '[FEATURE ROW 1 TITLE]', subtitle: '[FEATURE ROW 1 SUBTITLE]', desc: '[FEATURE ROW 1 DESC]', reverse: false },
  { eyebrow: '[EYEBROW 2]', title: '[FEATURE ROW 2 TITLE]', subtitle: '[FEATURE ROW 2 SUBTITLE]', desc: '[FEATURE ROW 2 DESC]', reverse: true  },
  { eyebrow: '[EYEBROW 3]', title: '[FEATURE ROW 3 TITLE]', subtitle: '',                          desc: '[FEATURE ROW 3 DESC]', reverse: false },
  { eyebrow: '[EYEBROW 4]', title: '[FEATURE ROW 4 TITLE]', subtitle: '',                          desc: '[FEATURE ROW 4 DESC]', reverse: true  },
]

const TESTIMONIALS = [
  { name: '[NAME 1]', role: '[ROLE 1]', flag: 'GB', headline: '[HEADLINE 1]', body: '[BODY 1]', avatarSrc: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=160&h=160&fit=crop&crop=faces' },
  { name: '[NAME 2]', role: '[ROLE 2]', flag: 'US', headline: '[HEADLINE 2]', body: '[BODY 2]', avatarSrc: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=160&h=160&fit=crop&crop=faces' },
  { name: '[NAME 3]', role: '[ROLE 3]', flag: 'AU', headline: '[HEADLINE 3]', body: '[BODY 3]', avatarSrc: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=160&h=160&fit=crop&crop=faces' },
  { name: '[NAME 4]', role: '[ROLE 4]', flag: 'DE', headline: '[HEADLINE 4]', body: '[BODY 4]', avatarSrc: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=160&h=160&fit=crop&crop=faces' },
  { name: '[NAME 5]', role: '[ROLE 5]', flag: 'CA', headline: '[HEADLINE 5]', body: '[BODY 5]', avatarSrc: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=160&h=160&fit=crop&crop=faces' },
  { name: '[NAME 6]', role: '[ROLE 6]', flag: 'FR', headline: '[HEADLINE 6]', body: '[BODY 6]', avatarSrc: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=160&h=160&fit=crop&crop=faces' },
  { name: '[NAME 7]', role: '[ROLE 7]', flag: 'NL', headline: '[HEADLINE 7]', body: '[BODY 7]', avatarSrc: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=160&h=160&fit=crop&crop=faces' },
  { name: '[NAME 8]', role: '[ROLE 8]', flag: 'US', headline: '[HEADLINE 8]', body: '[BODY 8]', avatarSrc: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=160&h=160&fit=crop&crop=faces' },
  { name: '[NAME 9]', role: '[ROLE 9]', flag: 'GB', headline: '[HEADLINE 9]', body: '[BODY 9]', avatarSrc: 'https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?w=160&h=160&fit=crop&crop=faces' },
  { name: '[NAME 10]', role: '[ROLE 10]', flag: 'SG', headline: '[HEADLINE 10]', body: '[BODY 10]', avatarSrc: 'https://images.unsplash.com/photo-1463453091185-61582044d556?w=160&h=160&fit=crop&crop=faces' },
  { name: '[NAME 11]', role: '[ROLE 11]', flag: 'BR', headline: '[HEADLINE 11]', body: '[BODY 11]', avatarSrc: 'https://images.unsplash.com/photo-1489424731084-a5d8b219a5bb?w=160&h=160&fit=crop&crop=faces' },
  { name: '[NAME 12]', role: '[ROLE 12]', flag: 'JP', headline: '[HEADLINE 12]', body: '[BODY 12]', avatarSrc: 'https://images.unsplash.com/photo-1499996860823-5214fcc65f8f?w=160&h=160&fit=crop&crop=faces' },
]
...
function TestimonialsSection() {
  ...
  useEffect(() => {
    ...
    if (!sentinel || autoLoadDone || visibleCount >= 9) return

    const observer = new IntersectionObserver(entries => {
      if (!entries[0]?.isIntersecting) return

      setVisibleCount(count => {
        const nextCount = Math.min(count + 3, 9)
        if (nextCount >= 9) setAutoLoadDone(true)
        return nextCount
      })
    })
  ...
  }, [autoLoadDone, visibleCount])

  return (
    <section style={{ background: 'var(--color-surface-raised)', padding: '100px var(--spacing-10)' }}>
      <h2 style={{ fontFamily: 'var(--font-family-title)', fontSize: 'clamp(36px, 4.5vw, 64px)', fontWeight: 'var(--font-weight-medium)', color: 'var(--color-text-title)', textAlign: 'center', letterSpacing: '-0.02em', lineHeight: '1.1', marginBottom: '80px' }}>
        [TESTIMONIALS HEADLINE]
      </h2>

      <div className="testimonials-grid" style={{ columnGap: 'var(--spacing-6)', maxWidth: '1200px', margin: '0 auto' }}>
        {visibleTestimonials.map(testimonial => (
          <div key={`${testimonial.name}-${testimonial.role}`} style={{ marginBottom: 'var(--spacing-6)', breakInside: 'avoid' }}>
      ...
      {autoLoadDone && hasMore && (
        <div style={{ display: 'flex', justifyContent: 'center', marginTop: 'var(--spacing-10)' }}>
          <Button variant="secondary" onClick={() => setVisibleCount(TESTIMONIALS.length)}>
            Load more
          </Button>
        </div>
      )}
    ...
    <>
      ...
      <section style={{ background: 'var(--color-surface-raised)', padding: '100px var(--spacing-10) 80px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-end', gap: 'var(--spacing-6)', marginBottom: 'var(--spacing-16)', flexWrap: 'wrap' }}>
          {[
            ...
            <div key={i} className="recognition-placeholder" style={{ width: p.w, height: p.h, borderRadius: 'var(--border-radius-xl)', background: 'var(--color-warm-200)', transform: `rotate(${p.rotate})` }} />
          ))}
        </div>
      </section>
      ...
      <section style={{ background: '#141414', padding: '128px var(--spacing-10)', textAlign: 'center' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          ...
          <div className="dark-cta-points" style={{ display: 'flex', justifyContent: 'center', gap: '80px', marginTop: 'var(--spacing-12)', flexWrap: 'wrap', marginBottom: '0' }}>
            {['[PROBLEM POINT 1]', '[PROBLEM POINT 2]', '[PROBLEM POINT 3]'].map(label => (
      ...
      <div style={{ padding: '80px 0 20px', background: 'var(--color-surface-page)' }}>
        ...
        {FEATURE_ROWS.map(f => (
          <section key={f.eyebrow} className={f.reverse ? 'feature-row feature-row-reverse' : 'feature-row'} style={{ background: 'var(--color-warm-100)', margin: '0 40px 60px', borderRadius: 'var(--border-radius-2xl)', padding: '80px 60px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '80px', alignItems: 'center', direction: f.reverse ? 'rtl' : 'ltr' }}>
            <div className="feature-content" style={{ direction: 'ltr' }}>
            ...
            <div className="feature-image" style={{ direction: 'ltr', background: 'var(--color-warm-300)', borderRadius: 'var(--border-radius-xl)', aspectRatio: '4/3', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
      ...
      {/* ── FOOTER ── */}
      <footer className="site-footer" style={{ background: 'var(--color-warm-200)', padding: '80px 60px 48px' }}>
        <div className="footer-grid" style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: '260px 1fr 1fr 1fr', gap: '40px', paddingBottom: '60px', borderBottom: '1px solid var(--color-border-subtle)' }}>
          <div className="footer-brand" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-6)' }}>
            <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-3)', textDecoration: 'none' }}>
              <span style={{ fontFamily: 'var(--font-family-title)', fontSize: 'var(--font-size-xl)', fontWeight: 'var(--font-weight-bold)', color: 'var(--color-text-title)', letterSpacing: '-0.02em' }}>designersbuild</span>
            </Link>
            <div style={{ display: 'flex', gap: 'var(--spacing-4)' }}>
              {[
                { href: '#', path: 'M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.253 5.622 5.91-5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z' },
          ...
          {[
            { heading: 'Product', links: [{ label: 'Blog', href: '#' }, { label: 'How it works', href: '#' }] },
            { heading: 'Support', links: [{ label: 'Contact us', href: '#' }, { label: 'FAQ', href: '#' }] },
            { heading: 'Legal',   links: [{ label: 'Privacy policy', href: '#' }, { label: 'Terms of use', href: '#' }] },
          ].map(col => (
            <div key={col.heading}>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', paddingTop: 'var(--spacing-8)' }}>
          <p style={{ fontFamily: 'var(--font-family-primary)', fontSize: 'var(--font-size-xs)', fontWeight: 'var(--font-weight-normal)', color: 'var(--color-text-muted)' }}>© 2026 designersbuild. All rights reserved.</p>
        </div>
      </footer>
    </>
  )
